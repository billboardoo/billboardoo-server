...
router.get('/search/keyword/:value', (req, res) => {
    ...
    db.all(`SELECT * FROM total WHERE title LIKE "%${value}%" OR artist LIKE "%${value}%" ORDER BY date DESC`, (err, rows) => {
        res.json(rows);
    });
});
...
router.get('/update/:type', (req, res) => {
    if (req.params.type === "all") {
        db.all(`SELECT * FROM updated WHERE type = "${req.params.type}"`, (err, rows) => {
